...
| Shared graph and warning foundations | In progress | Codex | Deterministic 30-factor hop layout and searchable textual graph alternative pass focused tests; a non-color Warning Center exposes limitations; and a reusable relationship inspector exposes/editably stages type, units, lag, evidence provenance, and state. The canonical app now renders the shared workspace shell. Pan/zoom, server-backed relationship edits, and evidence linking remain. |
