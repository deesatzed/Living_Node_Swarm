...
| Relationship inspector foundation | In progress | Codex | The reusable draft-only inspector exposes editable/reviewable relationship type, sign, transform, coefficient distribution, source/target units, lag and lag unit, validity range, evidence links, warnings, and proposal state while stating that active structure is unchanged until separate approval. Canonical Edit now offers explicit proposed-relationship controls for an existing dependency (type, transform, units, sign, lag, and coefficient units), stages the typed contract locally, and saves it in the exact version-bound candidate revision. The server validates it and it remains outside simulation and approval (68 server tests; 92 shared tests; shared/canonical builds; 6 fixture E2E tests, 2026-07-28). Evidence-link authoring, full reusable-inspector binding, and structural application remain. |
