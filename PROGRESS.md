...
| Existing-model Monitor mode | In progress | Codex | Monitor mode loads/saves project-scoped cadence, freshness threshold, and fixture/local/live mode through the persisted workspace API. Fixture events are explicitly labeled non-live and offer inspect, acknowledge, or branch-to-edit actions only; no event mutates model structure (33 shared tests, shared type build, and canonical production build, 2026-07-28). Durable acknowledgements, re-run action, and external polling remain. |
...
| Build target-to-map transition | In progress | Codex | After persisted target intake succeeds, the canonical New-project route moves to an operator-triggered candidate-map step. It loads only the explicit deterministic fixture endpoint and labels the 30-factor graph as fixture/not-live/proposal-only; no candidate becomes active (39 shared tests, shared type build, and canonical production build, 2026-07-28). Consented research, ledger persistence, live proposal routing, and refinement remain. |
| Canonical viewport smoke evidence | In progress | Codex | Fixture-intercepted Playwright Build-entry smoke tests pass at `1440x900` and `1280x800`; screenshots are saved in `docs/verification/gui/`. This proves Project Home → New project → target intake rendering only, not the complete Build/Run/Edit/Monitor journey or graph readability (2 E2E tests, 2026-07-28). |
| Shared graph and warning foundations | In progress | Codex | The fixture workspace now renders a target-centered, keyboard-selectable visual graph with deterministic 30-factor placement, search, textual state filtering, non-color selection status, and a full textual alternative. A non-color Warning Center exposes limitations; a reusable relationship inspector exposes/editably stages type, units, lag, evidence provenance, and state (37 shared tests and shared type build, 2026-07-28). This remains fixture-only and is not yet routed into the canonical project workspace; pan/zoom, relationship paths, server-backed edits, and evidence linking remain. |
