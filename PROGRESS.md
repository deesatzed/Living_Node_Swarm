...
| Candidate revision persistence | In progress | Codex | The workspace API persists typed, non-active revisions with exact base graph version, numeric parameter overrides, validated active/excluded node- and existing-dependency-state overrides, and kernel-validated proposed new nodes that require human approval. Stale bases, unknown node IDs/relationships/dependencies, and new-node ID collisions reject; revisions survive restart, and saving one leaves the active graph unchanged. Edit lists existing revisions, saves just-compared numeric overrides or explicitly staged node/dependency deltas, can stage a proposed Normal root factor with explicit ID/name/location/scale, load a matching-base revision back into local staging while clearing prior comparison/proposal state, and supports browser-local undo/redo of staged changes; structural deltas are visibly non-simulatable and non-approvable pending a separate structural-proposal contract (8 focused workspace tests; 78 shared tests; shared type build, 2026-07-28). New relationships and full revision validation remain. |
...
| Canonical viewport smoke evidence | In progress | Codex | Fixture-intercepted Playwright checks pass at `1440x900` and `1280x800` for Project Home → New project → target intake → durable Vet research categories and no-content-sent routing consent → explicit 15-factor fixture candidate map. The canonical suite also proves Monitor → read-only event inspection → draft-only Edit branch and Project Home → Run → authoritative outcome/stability receipt without structural mutation, followed by exact saved ensemble binding acknowledgment and server-receipt display. Screenshots are saved in `docs/verification/gui/`. These cases do not prove live research or complete Run/Edit/Monitor journeys (6 E2E tests, 2026-07-28). |
...
| Distribution inspector foundation | In progress | Codex | The reusable inspector renders all eight canonical families with a labeled curve preview, plain-language fit, support, canonical parameters, as-of/provenance state, and optional analytic values. In Edit it binds to the selected persisted graph factor’s actual family/parameters and now passes its recorded unit and explicit node support bounds through the inspector. A new read-only server statistics route normalizes documented legacy parameter names and returns kernel-derived mean/median/mode/variance; the UI displays defined values and standard deviation without fitting or mutating the node. Missing graph-node as-of/provenance remains explicit. Elicitation and quantitative revision persistence remain (2 focused server tests; 79 shared tests; shared type build, 2026-07-28). |
| Relationship inspector foundation | In progress | Codex | The reusable draft-only inspector exposes editable/reviewable relationship type, sign, transform, coefficient distribution, source/target units, lag and lag unit, validity range, evidence links, warnings, and proposal state while stating that active structure is unchanged until separate approval. Canonical Edit offers explicit proposed-relationship controls for an existing dependency (type, transform, units, sign, lag, coefficient units, and comma-separated local evidence-claim IDs), stages/removes the typed contract locally with undo/redo, and sends staged contracts to the server validator to display dependence warnings before persistence. An empty warning set now has an explicit clean-result receipt, and any staged-contract replacement, removal, undo/redo restore, or revision load clears the prior receipt so it cannot be misread as current. The server rejects dangling evidence claims; the actual fixture-intercepted Monitor → Edit browser journey proves the adapter wiring and warning receipt. Relationships remain outside simulation and approval (68 server tests; 95 shared tests; shared/canonical builds; 6 fixture E2E tests, 2026-07-28). Full reusable-inspector binding and structural application remain. |
