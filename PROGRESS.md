...
| Weighted mixture primitive | In progress | Codex | Kernel validates exact finite non-negative member weights, normalizes them, and creates a seeded mixture from member outcome samples rather than averaging point summaries. The server accepts 2–8 explicit graph/version/target/weight members, verifies every selected version, returns each member receipt plus the mixture distribution, and labels the result non-mutating/non-recommendatory. A project-scoped typed ensemble configuration persists exact unique graph/version/target bindings and positive-total weights across restart, validating all members against current graphs while preserving the active project graph/version; it is saved-for-review only. Run can save the exact visible two-model binding, list saved configurations, and restore one only when it binds the selected approved model; it repeats that nothing becomes approved or active. It also provides the backend mixture receipt and limitations without approval language (7 focused kernel ensemble tests; 10 focused workspace tests; 9 focused server API tests; 27 focused shared tests, 2026-07-28). Ensemble-specific approval remains. |
