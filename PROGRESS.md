...
| Candidate revision persistence | In progress | Codex | The workspace API persists typed, non-active revisions with exact base graph version, numeric parameter overrides, validated active/excluded node- and existing-dependency-state overrides, and kernel-validated proposed new nodes that require human approval. Stale bases, unknown node IDs/relationships/dependencies, and new-node ID collisions reject; revisions survive restart, and saving one leaves the active graph unchanged. Edit lists existing revisions, saves just-compared numeric overrides or explicitly staged node/dependency deltas, can load a matching-base revision back into local staging while clearing prior comparison/proposal state, and supports browser-local undo/redo of staged numeric/node/dependency changes; structural deltas are visibly non-simulatable and non-approvable pending a separate structural-proposal contract (8 focused workspace tests; 77 shared tests; shared type build, 2026-07-28). The UI does not yet stage new nodes; new relationships and full revision validation remain. |
