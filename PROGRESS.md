...
| Active-versus-candidate comparison | In progress | Codex | Existing-model Edit now loads the selected approved graph and permits a deliberate single numeric parameter override for an in-memory shadow comparison. It renders active/candidate mean and median, confirms active graph immutability, and states that a shift is structural impact rather than evidence of forecast improvement. The interaction is graph-generic and browser-fixture exercised, but does not persist a candidate revision, identify affected paths, compare models/ensembles, or permit approval (60 shared tests; shared/canonical type builds; 5 E2E tests; 55 server tests, 2026-07-28). |
