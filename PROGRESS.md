...
| Candidate revision persistence | In progress | Codex | The workspace API persists typed, non-active revisions with exact base graph version, numeric parameter overrides, and validated active/excluded node-state overrides. Stale bases and unknown node IDs reject, revisions survive restart, and saving one leaves the active graph unchanged. Edit now lists existing revisions and saves just-compared numeric overrides or explicitly staged node-state deltas through that API; structural deltas are visibly non-simulatable and non-approvable pending a separate structural-proposal contract (6 focused workspace tests; 69 shared tests; shared type build, 2026-07-28). Relationship deltas, restore/undo, and full revision validation remain. |
