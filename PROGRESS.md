...
| Candidate revision persistence | In progress | Codex | The workspace API persists a typed, non-active candidate revision with exact base graph version and parameter overrides. Stale bases reject, revisions survive restart, and saving one leaves the active graph unchanged. Edit now lists existing revisions and saves exactly the just-compared override set through that API; neither action activates the graph (5 focused workspace tests; 57 server tests; 65 shared tests; shared/canonical type builds; 5 E2E tests, 2026-07-28). Structural deltas, restore/undo, and revision validation remain. |
