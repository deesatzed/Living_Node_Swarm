...
| Shared Project Home and workspace shell | Complete | Codex | Accessible New/Run/Edit/Monitor actions, project truth cards, lifecycle guard, and visible target/version/freshness/fixture state are implemented in shared components. Project Home loads persisted projects plus their target-contract question/horizon and provides explicit operator selection, so existing-model actions bind the chosen project ID. New project persists an Idea-stage workspace before target intake. The canonical app opens through this shared router, with distinct build/run/edit/monitor workspace shells. Server-backed local/live evidence classifications are typed (36 shared tests, shared type build, and canonical production build, 2026-07-28). Full run/edit/monitor controls and receipts remain. |
| Existing-model Run mode | In progress | Codex | Run mode calls the authoritative graph simulation endpoint only for the selected project's approved graph and renders its snapshot ID, graph version, seed, sample count, and freshness receipt. It has no structural-edit control; missing approved graphs show an explicit limitation. Named project scenarios persist separately from active graph structure and can record assumptions for later comparison (43 shared tests, shared type build, and canonical production build, 2026-07-28). Scenario application, prior-run comparison, sensitivity, and durable receipt history remain. |
| Existing-model Monitor mode | In progress | Codex | Monitor mode loads/saves project-scoped cadence, freshness threshold, and fixture/local/live mode through the persisted workspace API. Fixture events are explicitly labeled non-live; acknowledgement persists a timestamped receipt without mutating model structure. Inspect/branch-to-edit controls, re-run action, and external polling remain (43 shared tests, shared type build, and 3 focused server tests, 2026-07-28). |
| Existing-model Edit mode | In progress | Codex | Edit mode refuses to start without an approved graph version, otherwise creates a typed server-persisted draft bound to that exact version before any candidate work. The UI states that active structure remains unchanged until separate review/approval (35 shared tests, shared type build, and canonical production build, 2026-07-28). Reversible refinement commands, shadow comparison, approval invalidation, and revision deltas remain. |
| Idea/Vet component foundation | In progress | Codex | Accessible target form distinguishes the Neodymium private-investor retail series, blocks incomplete contracts, collects every authoritative target-contract field with timezone-aware submission values, and has a reusable project-persistence wrapper that reports success/failure without attaching a failed target. In canonical Vet, Pause/Resume works, action-specific discovery entries persist as typed project-ledger records, and the operator can persist a local-only research preference that records no content leaves the Mac; Proceed now does not grant provider consent (54 shared tests, 55 server tests, shared/canonical type builds, and 4 E2E tests, 2026-07-28). Provider-consent receipts, research brief categories, and live research remain. |
| Build target-to-map transition | In progress | Codex | After persisted target intake succeeds, the canonical New-project route moves to an operator-triggered candidate-map step. It loads only the explicit deterministic fixture endpoint and labels the graph fixture/not-live/proposal-only; no candidate becomes active. The canonical browser fixture contains 15 factors and one visible three-hop proposal path. Consented research, ledger persistence, live proposal routing, and refinement remain (44 shared tests, shared type build, canonical production build, 2026-07-28). |
| Canonical viewport smoke evidence | In progress | Codex | Fixture-intercepted Playwright checks pass at `1440x900` and `1280x800` for Project Home → New project → target intake and the 15-factor target persistence → Vet → explicit fixture candidate map. Screenshots are saved in `docs/verification/gui/`. The fixture-map cases prove a labeled proposal-only graph and its traced path render at both desktop viewports, not live research or complete Build/Run/Edit/Monitor journeys (4 E2E tests, 2026-07-28). |
| Shared graph and warning foundations | In progress | Codex | The fixture candidate map reached from the canonical Build route renders a target-centered, keyboard-selectable visual graph with deterministic no-default-overlap hop placement, search, state and hop filtering, drag pan, bounded zoom, fit-to-view, selected three-hop relationship trace, non-color selection status, and a full textual alternative. A non-color Warning Center exposes limitations; a reusable relationship inspector exposes/editably stages type, units, lag, evidence provenance, and state (44 shared tests and shared type build, 2026-07-28). This remains fixture-only; richer relationship paths, server-backed edits, and evidence linking remain. |
| Distribution inspector foundation | In progress | Codex | The reusable inspector now renders all eight canonical families with a labeled curve preview, plain-language fit, support, canonical parameters, as-of/provenance state, and optional mean/median/central interval values (52 shared tests and shared type build, 2026-07-28). It is not yet bound to factor-specific persisted distributions or quantitative review/approval in the canonical workflow. |
