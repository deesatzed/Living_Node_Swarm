...
| Weighted mixture primitive | In progress | Codex | Kernel validates exact finite non-negative member weights, normalizes them, and creates a seeded mixture from member outcome samples rather than averaging point summaries. The server now accepts 2–8 explicit graph/version/target/weight members, verifies every selected version, returns each member receipt plus the mixture distribution, and labels the result non-mutating/non-recommendatory/non-persisted. Ensemble persistence/approval and UI review remain (7 focused kernel ensemble tests; 9 focused server API tests, 2026-07-28). |
