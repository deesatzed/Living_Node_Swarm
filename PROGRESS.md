...
| Canonical viewport smoke evidence | In progress | Codex | Fixture-intercepted Playwright checks pass at `1440x900` and `1280x800` for Project Home → New project → target intake → durable Vet research categories and no-content-sent routing consent → explicit 15-factor fixture candidate map. The canonical suite also proves Monitor → read-only event inspection → draft-only Edit branch and Project Home → Run → authoritative outcome/stability receipt without structural mutation. Screenshots are saved in `docs/verification/gui/`. These cases do not prove live research or complete Run/Edit/Monitor journeys (6 E2E tests, 2026-07-28). |
