...
| Shared Project Home and workspace shell | Complete | Codex | Accessible New/Run/Edit/Monitor actions, project truth cards, lifecycle guard, and visible target/version/freshness/fixture state are implemented in shared components. Existing-model actions now bind the selected persisted project ID, and server-backed local/live evidence classifications are typed (focused tests and shared type build, 2026-07-28). Canonical-app loading/routing remains. |
