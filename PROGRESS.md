...
| Weighted mixture primitive | In progress | Codex | Kernel validates exact finite non-negative member weights, normalizes them, and creates a seeded mixture from member outcome samples rather than averaging point summaries. The server accepts 2–8 explicit graph/version/target/weight members, verifies every selected version, returns each member receipt plus the mixture distribution, and labels the result non-mutating/non-recommendatory/non-persisted. Run provides a review surface that pre-binds the selected approved graph/version and requires an explicit alternative graph/version/target and two weights before it calls the backend; it shows mixture/member receipts and limitations without approval language (7 focused kernel ensemble tests; 9 focused server API tests; 26 focused shared tests, 2026-07-28). Ensemble persistence/approval remain. |
