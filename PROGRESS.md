...
| Relationship inspector foundation | In progress | Codex | The reusable draft-only inspector now exposes editable/reviewable relationship type, sign, transform, coefficient distribution, source/target units, lag and lag unit, validity range, evidence links, warnings, and proposal state while stating that active structure is unchanged until separate approval. A complete proposed kernel `RelationshipContract` can now persist across restart inside a version-bound candidate revision after its parent/child IDs are checked against the exact active graph; it remains outside simulation and approval (68 server tests; 54 shared tests and shared/canonical type builds, 2026-07-28). Canonical inspector-to-revision wiring remains. |
