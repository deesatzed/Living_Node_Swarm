...
| Weighted mixture primitive | In progress | Codex | Kernel now validates exact finite non-negative member weights, normalizes them, and creates a seeded mixture from member outcome samples rather than averaging point summaries. The primitive returns a distributional predictive payload and normalized weights; graph-version binding, persistence/approval, and UI review remain (7 focused kernel ensemble tests, 2026-07-28). |
