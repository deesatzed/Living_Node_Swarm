...
export function VettingConversation({ provider, model, dataScope, onProceed, onRecord, onRecordLocalOnly, onRecordProviderRouting, onRecordResearchCategories }: {
  ...
  return <section aria-labelledby="vetting-title">
    ...
    {action && <section aria-label="Record discovery action"><label>{action.prompt}<input value={text} onChange={(event) => setText(event.target.value)} /></label><button onClick={() => void record()} disabled={busy || !text.trim()}>{busy ? "Saving discovery entry…" : "Save discovery action"}</button><button onClick={() => { setAction(null); setText(""); setStatus("Discovery action cancelled. No entry was saved."); }}>Cancel discovery action</button></section>}
