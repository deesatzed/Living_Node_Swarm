...
export function NewProjectFlow({ client, onCreated }: { client: NewProjectClient; onCreated: (projectId: string) => void }) {
  ...
  if (projectId && targetId && buildStage === "map") return <section aria-label="Build candidate map"><h1>Map proposed factors</h1><p>Target contract saved. Continue through the explicitly labeled candidate map before any approval.</p><CandidateMap targetId={targetId} projectId={projectId} client={client} onMaterialized={async (graphId) => { await client.patchProject(projectId, { graph_id: graphId, stage: "map" }); }} /></section>;
  ...
  async function recordProviderRoutingConsent(receipt: { mode: string; provider: string; model: string; data_scope: string; confirmed_at: string }) {
    if (!projectId) return;
    await client.patchProject(projectId, { research_consent: receipt });
  }
  async function recordResearchCategories(categories: string[]) {
    if (!projectId) return;
    const next = [...vetEntries, ...categories.map((category) => ({ classification: "scenario" as const, text: `Planned research category: ${category}` }))];
    await client.patchProject(projectId, { discovery_ledger: next });
    setVetEntries(next);
  }
