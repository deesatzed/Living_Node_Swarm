...
export function ProjectWorkspaceRouter({ client }: { client: ProjectWorkspaceClient }) {
  ...
  if (route.kind !== "home") return <ExistingProjectWorkspace mode={route.kind} projectId={route.projectId} client={client} onBack={() => setRoute({ kind: "home" })} onBranchToEdit={() => setRoute({ kind: "edit", projectId: route.projectId })} onOpenRun={() => setRoute({ kind: "run", projectId: route.projectId })} />;
