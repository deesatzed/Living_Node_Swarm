...
export function ShadowComparison({ graphId, projectId, activeGraphVersion, client, onApproved }: { graphId: string; projectId?: string; activeGraphVersion?: number; client: ShadowComparisonClient; onApproved?: (project: JsonObject) => void }) {
  ...
  function stageRelationshipContract() {
    ...
    captureStaging(); setStagedRelationshipContracts((current) => [...current.filter((item) => item.id !== id), contract]); setRelationshipValidation(null); setError("");
  }
  function removeStagedRelationshipContract(id: string) { captureStaging(); setStagedRelationshipContracts((current) => current.filter((relationship) => relationship.id !== id)); setRelationshipValidation(null); }
