...
interface StagingSnapshot { overrides: Record<string, Record<string, number>>; nodeStates: Record<string, "active" | "excluded">; relationshipStates: Record<string, "active" | "excluded">; relationshipContracts: JsonObject[]; newNodes: JsonObject[]; }

function snapshot(overrides: Record<string, Record<string, number>>, nodeStates: Record<string, "active" | "excluded">, relationshipStates: Record<string, "active" | "excluded">, relationshipContracts: JsonObject[], newNodes: JsonObject[]): StagingSnapshot {
  return { overrides: Object.fromEntries(Object.entries(overrides).map(([nodeId, parameters]) => [nodeId, { ...parameters }])), nodeStates: { ...nodeStates }, relationshipStates: { ...relationshipStates }, relationshipContracts: relationshipContracts.map((relationship) => ({ ...relationship })), newNodes: newNodes.map((node) => ({ ...node, parameters: node.parameters && typeof node.parameters === "object" ? { ...(node.parameters as JsonObject) } : node.parameters })) };
}
...
export function ShadowComparison({ graphId, projectId, activeGraphVersion, client, onApproved }: { graphId: string; projectId?: string; activeGraphVersion?: number; client: ShadowComparisonClient; onApproved?: (project: JsonObject) => void }) {
  ...
  function captureStaging() { setStagingHistory((current) => [...current, snapshot(stagedOverrides, stagedNodeStates, stagedRelationshipStates, stagedRelationshipContracts, stagedNewNodes)]); setStagingRedo([]); }
  function restoreStaging(next: StagingSnapshot) { setStagedOverrides(next.overrides); setStagedNodeStates(next.nodeStates); setStagedRelationshipStates(next.relationshipStates); setStagedRelationshipContracts(next.relationshipContracts); setStagedNewNodes(next.newNodes); setRelationshipValidation(null); }
  function undoStaging() { const previous = stagingHistory.at(-1); if (!previous) return; setStagingHistory((current) => current.slice(0, -1)); setStagingRedo((current) => [...current, snapshot(stagedOverrides, stagedNodeStates, stagedRelationshipStates, stagedRelationshipContracts, stagedNewNodes)]); restoreStaging(previous); }
  function redoStaging() { const next = stagingRedo.at(-1); if (!next) return; setStagingRedo((current) => current.slice(0, -1)); setStagingHistory((current) => [...current, snapshot(stagedOverrides, stagedNodeStates, stagedRelationshipStates, stagedRelationshipContracts, stagedNewNodes)]); restoreStaging(next); }
  ...
  function stageRelationshipContract() {
    ...
    captureStaging(); setStagedRelationshipContracts((current) => [...current.filter((item) => item.id !== id), contract]); setRelationshipValidation(null); setStructuralProposal(null); setStructuralApproval(null); setError("");
  }
  function removeStagedRelationshipContract(id: string) { captureStaging(); setStagedRelationshipContracts((current) => current.filter((relationship) => relationship.id !== id)); setRelationshipValidation(null); setStructuralProposal(null); setStructuralApproval(null); }
  ...
  async function createStructuralProposal() {
    if (!client.createStructuralProposal || stagedRelationshipContracts.length === 0) return;
    setError("");
    try {
      const response = await client.createStructuralProposal(graphId, { relationships: stagedRelationshipContracts });
      ...
      setStructuralProposal(saved as JsonObject); setStructuralApproval(null); setStructuralReviewed(false);
  ...
  async function approveStructuralProposal() {
    ...
    if (!client.approveStructuralProposal || !proposalId || !bindingHash || !structuralApprover.trim() || !structuralReviewed) return;
    setError("");
    try { setStructuralApproval(await client.approveStructuralProposal(graphId, proposalId, { approved_by: structuralApprover.trim(), binding_hash: bindingHash })); }
  ...
  async function saveCandidateRevision() {
    ...
    if (!projectId || !activeGraphVersion || !client.createCandidateRevision || (!Object.keys(parameterOverrides).length && !Object.keys(stagedNodeStates).length && !Object.keys(stagedRelationshipStates).length && !stagedRelationshipContracts.length && !stagedNewNodes.length)) return;
    setError("");
    try {
  ...
  return <section aria-label="Active versus candidate comparison">
    ...
    {nodes.length > 0 && <>
      ...
      {stagedRelationshipContracts.length > 0 && client.createStructuralProposal && !structuralProposal && <button onClick={createStructuralProposal}>Create structural proposal for review</button>}
      {structuralProposal && <section aria-label="Structural proposal review"><h3>Structural proposal review</h3><p>Proposal {numeric(structuralProposal.id)} · graph version {numeric(structuralProposal.graph_version)}</p><p>Binding hash: {numeric(structuralProposal.binding_hash)}</p><p>This review concerns structural impact, not forecast accuracy. The active graph remains unchanged until exact approval.</p><label>Structural approver identity<input aria-label="Structural approver identity" value={structuralApprover} onChange={(event) => setStructuralApprover(event.target.value)} /></label><label><input aria-label="I reviewed this structural binding" type="checkbox" checked={structuralReviewed} onChange={(event) => setStructuralReviewed(event.target.checked)} />I reviewed this structural binding</label>{client.approveStructuralProposal && <button onClick={approveStructuralProposal} disabled={!structuralApprover.trim() || !structuralReviewed || typeof structuralProposal.binding_hash !== "string"}>Approve structural proposal</button>}</section>}
