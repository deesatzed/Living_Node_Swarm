...
function graphNodes(graph: JsonObject): GraphNode[] {
  ...
  return Object.entries(graph.nodes as Record<string, unknown>).flatMap(([id, raw]) => {
    ...
    return [{ id, name: typeof node.name === "string" ? node.name : id, family: typeof node.distribution_family === "string" ? node.distribution_family : undefined, provenance, classification: typeof node.evidence_classification === "string" ? node.evidence_classification : "Not recorded on graph node", asOf: typeof distributionSpec?.as_of === "string" ? distributionSpec.as_of : "Not recorded on graph node", units: typeof node.units === "string" ? node.units : "Not recorded on graph node", support, parameters, dependsOn: Array.isArray(node.depends_on) ? node.depends_on.filter((item): item is string => typeof item === "string") : [] }];
  });
}
...
export function ShadowComparison({ graphId, projectId, activeGraphVersion, client, onApproved, onEvidenceClaimIds }: { graphId: string; projectId?: string; activeGraphVersion?: number; client: ShadowComparisonClient; onApproved?: (project: JsonObject) => void; onEvidenceClaimIds?: (claimIds: string[]) => void }) {
  ...
  return <section aria-label="Active versus candidate comparison">
    ...
    {nodes.length > 0 && <>
      ...
      <section aria-label="Candidate relationship change set"><h3>Candidate relationship change set</h3><label>Candidate dependency<select aria-label="Candidate dependency" value={selectedRelationship} onChange={(event) => { const edge = event.target.value; setSelectedRelationship(edge); const raw = activeRelationshipsByEdge[edge]; onEvidenceClaimIds?.(raw && Array.isArray(raw.evidence_claim_ids) ? raw.evidence_claim_ids.filter((claim): claim is string => typeof claim === "string" && Boolean(claim.trim())) : []); }}>{nodes.flatMap((node) => node.dependsOn.map((parent) => <option key={`${parent}:${node.id}`} value={`${parent}:${node.id}`}>{nodes.find((item) => item.id === parent)?.name ?? parent} → {node.name}</option>))}</select></label><button onClick={() => stageSelectedRelationshipState("excluded")}>Exclude selected dependency in candidate</button><button onClick={() => stageSelectedRelationshipState("active")}>Include selected dependency in candidate</button>{Object.keys(stagedRelationshipStates).length === 0 ? <p>No local candidate relationship-state changes staged.</p> : <ul>{Object.entries(stagedRelationshipStates).map(([relationship, state]) => <li key={relationship}>{relationship}: {state} <button onClick={() => removeStagedRelationshipState(relationship)}>Remove {relationship} state change</button></li>)}</ul>}<p>Relationship-state changes are revision-only and cannot be shadow-simulated or approved until a structural proposal contract exists.</p></section>
      ...
      {selected?.family && <DistributionInspector family={selected.family} parameters={selected.parameters} support={selected.support} units={selected.units} asOf={selected.asOf} provenance={selected.provenance} classification={selected.classification} derived={derived ? { mean: derived.statistics.mean ?? undefined, median: derived.statistics.median ?? undefined, mode: derived.statistics.mode ?? undefined, standardDeviation: derived.statistics.variance === null ? undefined : Math.sqrt(derived.statistics.variance), interval: derived.display_quantiles ? `P05 ${derived.display_quantiles.p05} · P50 ${derived.display_quantiles.p50} · P95 ${derived.display_quantiles.p95}${derived.display_quantile_method === "seeded_monte_carlo_registry_sampler" ? " (seeded Monte Carlo display)" : " (server display quantiles)"}` : undefined } : undefined} />}
    </>}
