...
import type { CandidateApprovalInput, CandidateProposalInput, DistributionStatisticsResult, ElicitDistributionInput, JsonObject, ShadowSimulationInput, StructuralProposalInput, StructuralShadowSimulationInput, WorkspaceCandidateRevisionInput } from "../api/types";
...
export function ShadowComparison({ graphId, projectId, activeGraphVersion, client, onApproved }: { graphId: string; projectId?: string; activeGraphVersion?: number; client: ShadowComparisonClient; onApproved?: (project: JsonObject) => void }) {
  ...
  return <section aria-label="Active versus candidate comparison">
    ...
    {result && client.createCandidateProposal && !proposal && Object.keys(stagedDistributionSpecs).length === 0 && <button onClick={saveCandidate}>Save candidate for review</button>}
