...
export function ShadowComparison({ graphId, projectId, activeGraphVersion, client, onApproved }: { graphId: string; projectId?: string; activeGraphVersion?: number; client: ShadowComparisonClient; onApproved?: (project: JsonObject) => void }) {
  ...
  async function createStructuralProposal() {
    const removedRelationshipIds = Object.entries(stagedRelationshipStates)
      .filter(([, state]) => state === "excluded")
      .flatMap(([edge]) => activeRelationshipIdsByEdge[edge] ?? []);
    if (!client.createStructuralProposal || (stagedRelationshipContracts.length === 0 && removedRelationshipIds.length === 0)) return;
    setError("");
    try {
      const response = await client.createStructuralProposal(graphId, { relationships: stagedRelationshipContracts, removed_relationship_ids: removedRelationshipIds });
  ...
  return <section aria-label="Active versus candidate comparison">
    ...
    {nodes.length > 0 && <>
      ...
      {(stagedRelationshipContracts.length > 0 || Object.entries(stagedRelationshipStates).some(([edge, state]) => state === "excluded" && Boolean(activeRelationshipIdsByEdge[edge]))) && client.createStructuralProposal && !structuralProposal && <button onClick={createStructuralProposal}>Create structural proposal for review</button>}
