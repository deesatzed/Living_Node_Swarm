...
export function ShadowComparison({ graphId, projectId, activeGraphVersion, client, onApproved }: { graphId: string; projectId?: string; activeGraphVersion?: number; client: ShadowComparisonClient; onApproved?: (project: JsonObject) => void }) {
  ...
  function restoreStaging(next: StagingSnapshot) { setStagedOverrides(next.overrides); setStagedNodeStates(next.nodeStates); setStagedRelationshipStates(next.relationshipStates); setStagedRelationshipContracts(next.relationshipContracts); setStagedNewNodes(next.newNodes); }
  ...
  function stageRelationshipContract() {
    const [parentNodeId, childNodeId] = selectedRelationship.split(":");
    const lagPeriods = Number(relationshipLagPeriods);
    if (!parentNodeId || !childNodeId || !Number.isInteger(lagPeriods) || lagPeriods < 0 || !relationshipSourceUnit.trim() || !relationshipTargetUnit.trim() || !relationshipCoefficientUnits.trim()) { setError("Choose a dependency and provide finite relationship units plus a non-negative whole lag."); return; }
  ...
  function loadCandidateRevision(revision: JsonObject) {
    ...
    setStagedOverrides(parameterOverrides); setStagedNodeStates(nodeStates); setStagedRelationshipStates(relationshipStates); setStagedRelationshipContracts(relationshipContracts); setStagedNewNodes(newNodes); setStagingHistory([]); setStagingRedo([]); setResult(null); setComparedOverrides(null); setProposal(null); setApproval(null); setReviewed(false); setError(""); setRevisionStatus(`Revision ${numeric(revision.id)} loaded into local staging. The active graph is unchanged.`);
  }
