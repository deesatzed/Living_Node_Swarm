...
import type { CandidateApprovalInput, CandidateProposalInput, DistributionStatisticsResult, JsonObject, ShadowSimulationInput, StructuralProposalInput, StructuralShadowSimulationInput, WorkspaceCandidateRevisionInput } from "../api/types";
...
export function ShadowComparison({ graphId, projectId, activeGraphVersion, client, onApproved }: { graphId: string; projectId?: string; activeGraphVersion?: number; client: ShadowComparisonClient; onApproved?: (project: JsonObject) => void }) {
  ...
  async function approveStructuralProposal() {
    ...
    setError("");
    try { setStructuralApproval(await client.approveStructuralProposal(graphId, proposalId, { approved_by: structuralApprover.trim(), binding_hash: bindingHash })); setStructuralComparison(null); }
