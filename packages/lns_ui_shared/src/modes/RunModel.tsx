...
export function RunModel({ graphId, client, projectId, scenarioClient, targetNodeId, activeGraphVersion, onReceipt }: { graphId: string; client: RunModelClient; projectId?: string; scenarioClient?: ScenarioClient; targetNodeId?: string; activeGraphVersion?: number; onReceipt?: (result: JsonObject) => Promise<void> }) {
  ...
  const [ensembleRationale, setEnsembleRationale] = useState("");
  ...
  async function saveMixture() {
    ...
    if (!projectId || !client.createEnsemble || !ensembleName.trim() || !ensembleRationale.trim() || !members) { setError("Provide an ensemble name, decision rationale, and two valid member bindings before saving."); return; }
    const id = typeof crypto !== "undefined" && crypto.randomUUID ? crypto.randomUUID() : `ensemble-${Date.now()}`;
    try { const saved = await client.createEnsemble(projectId, { id, name: ensembleName.trim(), rationale: ensembleRationale.trim(), members }); setSavedEnsembles((current) => [...current, saved]); setEnsembleStatus(`Ensemble ${ensembleName.trim()} saved for review only; it is not approved or active.`); setError(""); }
  ...
  function loadEnsemble(ensemble: JsonObject) {
    ...
    setCurrentWeight(String(current.weight)); setAlternativeGraphId(text(alternative.graph_id, "")); setAlternativeGraphVersion(text(alternative.graph_version, "")); setAlternativeTargetId(text(alternative.target_node_id, "")); setAlternativeWeight(String(alternative.weight)); setEnsembleName(text(ensemble.name, "")); setEnsembleRationale(text(ensemble.rationale, "")); setEnsembleStatus(`Loaded ${text(ensemble.name, "ensemble")} for review. It is not approved or active.`); setError("");
  }
  ...
  return <section aria-label="Run approved model controls">
    ...
    {targetNodeId && activeGraphVersion && client.runWeightedEnsemble && <section aria-label="Weighted model mixture">
      ...
      {projectId && client.createEnsemble && <><label>Ensemble configuration name<input aria-label="Ensemble configuration name" value={ensembleName} onChange={(event) => setEnsembleName(event.target.value)} /></label><label>Ensemble decision rationale<textarea aria-label="Ensemble decision rationale" value={ensembleRationale} onChange={(event) => setEnsembleRationale(event.target.value)} /></label><button onClick={saveMixture}>Save ensemble for review</button>{ensembleStatus && <p role="status">{ensembleStatus}</p>}</>}
