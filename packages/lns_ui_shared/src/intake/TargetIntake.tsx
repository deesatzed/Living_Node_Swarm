...
const REQUIRED = ["id", "question", "forecast_origin", "price_basis", "unit", "resolution_at", "observation_rule", "missing_source_fallback", "revision_policy"] as const;
...
export function TargetIntake({ onSubmit }: { onSubmit: (target: Partial<TargetContractInput>) => void }) {
  ...
  function submit(event: FormEvent) {
    ...
    onSubmit({
      ...values,
      forecast_origin: new Date(values.forecast_origin!).toISOString(),
      resolution_at: new Date(values.resolution_at!).toISOString(),
    });
  }
  return <form onSubmit={submit} aria-labelledby="target-intake-title">
    ...
    <label>Target identifier<input name="id" value={values.id ?? ""} onChange={set("id")} aria-invalid={errors.some((error) => error.startsWith("id"))} /></label>
    ...
    <label>Forecast origin<input name="forecast_origin" type="datetime-local" value={values.forecast_origin ?? ""} onChange={set("forecast_origin")} aria-invalid={errors.some((error) => error.startsWith("forecast origin"))} /></label>
    ...
    <label>Observation rule<input name="observation_rule" value={values.observation_rule ?? ""} onChange={set("observation_rule")} aria-invalid={errors.some((error) => error.startsWith("observation rule"))} /></label>
