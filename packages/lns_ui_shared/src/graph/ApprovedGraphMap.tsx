...
export function ApprovedGraphMap({ graph }: { graph: JsonObject }) {
  ...
  return <section aria-label="Approved model dependency graph"><h2>Approved graph — read-only</h2><p>Trace the active dependencies before creating a separate draft. The approved graph itself is not edited here.</p><HopGraph factors={factors(nodes, targetNode.id)} targetId={targetNode.id} targetLabel={targetNode.name} relationships={nodes.flatMap((node) => node.dependsOn.map((parent) => ({ parent_node_id: parent, child_node_id: node.id })))} /></section>;
}
