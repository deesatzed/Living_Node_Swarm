...
| Q4-05 | Build relationship inspector | Q3-05, Q4-01 | type, units, lag, evidence, state editable/reviewable | In progress — draft-only reusable inspector exposes complete relationship semantics and warnings. Canonical Edit distinguishes existing dependency-state controls from proposed relationship additions and removal of an exact persisted active relationship: an operator supplies typed finite coefficient data for additions or excludes a metadata-backed dependency for removal, receives a server-issued structural proposal hash, runs its stored proposal as an in-memory structural comparison, and can approve only that reviewed binding. It clears approved structural staging and its fixture-intercepted browser journey proves remove → compare → approve → alternate-addition. Both comparison and validation receipts make non-mutation explicit and label output movement as structural impact, not accuracy; project-scoped approval updates the confirmed workspace lifecycle/version after graph acceptance and rejects stale alignment before mutation. Node deletion/generic state-delta approval and full reusable-inspector binding remain (76 server tests; 98 shared tests; 6 fixture E2E tests; 78 kernel tests, 2026-07-28). |
...
| Q4-07 | Build active-vs-candidate and model/ensemble comparison | Q3-06, Q4-01 | paths and distribution changes visible; explicit two-model weighted ensemble is backend-owned and no false improvement label appears | In progress — Edit stages/removes a graph-derived multi-parameter local candidate set, runs it in memory with paired summaries/affected path/no-improvement limitation, and can persist exact numeric overrides or explicit active/excluded node/dependency-state deltas as non-active version-bound revisions. Run can execute a saved, version-bound numeric scenario and compare/save/reload two operator-specified graph versions as a backend-sampled weighted distribution mixture without a false improvement/recommendation label. A named exact-binding ensemble approval now persists and is read-only reviewable after refresh, without activating an ensemble or mutating a graph. Active graphs now persist relationship metadata, and a server-issued structural relationship-addition proposal validates and structurally shadow-simulates a zero-lag trial graph without mutation; new structural elements and restore/undo remain (2026-07-28) |
