...
| Q4-05 | Build relationship inspector | Q3-05, Q4-01 | type, units, lag, evidence, state editable/reviewable | In progress — draft-only reusable inspector exposes complete relationship semantics and warnings. Canonical Edit distinguishes existing dependency-state controls from proposed relationship additions: an operator selects parent/child factors, supplies typed finite coefficient data, validates it, receives a server-issued structural proposal hash, runs its stored proposal as a zero-lag in-memory structural comparison, and can approve only that reviewed binding. Both comparison and validation receipts make non-mutation explicit and label output movement as structural impact, not accuracy; project-scoped approval updates the confirmed workspace lifecycle/version after graph acceptance and rejects stale alignment before mutation. Node/removal/state-delta approval and full reusable-inspector binding remain (74 server tests; 97 shared tests; 6 fixture E2E tests; 78 kernel tests, 2026-07-28). |
