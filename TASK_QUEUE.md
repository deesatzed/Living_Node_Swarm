...
| Q4-03 | Build target-centered hop graph | Q3-03, Q4-01 | 30-node deterministic layout has no default overlap | In progress — canonical 30-factor deterministic fixture and selected approved Edit graph support no-default-overlap hop layers, pan/zoom/fit, filtering, keyboard selection, three-hop tracing, and textual alternative. Approved-map edge details now use persisted active relationship contracts for type, transform, sign, units, coefficient parameters, lag, evidence claims, and state; missing legacy values remain explicit. Browser proof asserts 30 unique rendered coordinates at both `1440x900` and `1280x800`; graph-node evidence linking remains (2026-07-28) |
