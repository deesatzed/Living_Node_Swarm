...
| Q4-04 | Build node distribution inspector | Q1-01–04, Q4-01 | eight family forms/curves/derived values tested | In progress — reusable eight-family inspector is bound to the selected persisted Edit graph node and displays recorded units/support plus kernel-derived analytic values through a read-only statistics route. Same-family Normal/LogNormal median/P90 elicitation stages a provenance-bearing non-active candidate revision and an in-memory numeric equivalent, while withholding generic numeric approval. A server-owned route now provides named intuitive derivations for every canonical family, and Edit renders/stages the six remaining family forms through that route. Curve editing, all-form browser proof, fitting, and distribution-spec activation remain (88 server tests; 106 shared tests; 2026-07-28). |
