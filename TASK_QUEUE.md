...
| Q4-03 | Build target-centered hop graph | Q3-03, Q4-01 | 30-node deterministic layout has no default overlap | In progress — canonical 30-factor deterministic fixture and selected approved Edit graph support no-default-overlap hop layers, pan/zoom/fit, filtering, keyboard selection, three-hop tracing, and textual alternative. Browser proof asserts 30 unique rendered coordinates at both `1440x900` and `1280x800`; richer relationship/evidence integration remains (2026-07-28) |
...
| Q4-05 | Build relationship inspector | Q3-05, Q4-01 | type, units, lag, evidence, state editable/reviewable | In progress — draft-only reusable inspector exposes complete relationship semantics and warnings. Canonical Edit supports typed zero-lag relationship addition, exact persisted relationship removal, and metadata-complete non-target node retirement: each receives a server-issued structural proposal hash, may run only as an in-memory structural comparison, and requires exact reviewed approval. A retirement removes every incident active relationship before changing node status; incomplete/legacy metadata refuses partial activation. Receipts name added, removed, and retired structure, approved staging clears, and project-scoped approval updates confirmed lifecycle/version after graph acceptance. The fixture browser proof covers retirement → compare → approval. Physical deletion/restoration, generic state-delta approval, and full reusable-inspector binding remain (78 server tests; 100 shared tests; 7 fixture E2E tests; 78 kernel tests, 2026-07-28). |
