...
| Q4-04 | Build node distribution inspector | Q1-01–04, Q4-01 | eight family forms/curves/derived values tested | In progress — reusable eight-family inspector is bound to the selected persisted Edit graph node and displays recorded units/support plus kernel-derived analytic values through a read-only statistics route. Same-family Normal/LogNormal median/P90 elicitation stages a provenance-bearing non-active candidate revision and an in-memory numeric equivalent. A server-owned route provides named intuitive derivations for every canonical family, and Edit renders/stages the six remaining family forms through that route; component coverage verifies the intended human-facing input set for every non-quantile form, while the fixture browser journey sends all six non-quantile forms through the derivation endpoint. A typed extension to the exact-binding candidate proposal atomically activates matching parameters and the full provenance specification on approval; the inspector immediately refreshes as-of/support/method/confidence/evidence details from an authoritative returned graph. Fixture browser proof covers proposal review for Normal and Gamma. Curve editing and fitting remain (89 server tests; 113 shared tests; 8 fixture E2E tests; 2026-07-28). |
