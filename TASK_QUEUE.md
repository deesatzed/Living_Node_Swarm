...
| Q6-02 | Replace duplicate gas UI with shared workspace preset | Q4-10, Q6-01 | gas app uses shared components | In progress — the Gas preset mounts the shared `WorkspaceShell`, renders its dynamic dependency map through shared `HopGraph`, and now mounts the shared `DistributionInspector` for every selected Gas node. The inspector renders authoritative Gas simulation summary values when present and explicitly labels missing adapter support/units/as-of/source fields rather than inferring them. Gas-specific bootstrap, editable parameter controls, relationship inspection, and Kalshi-bound controls remain preset-specific. (gas build and safety verifier, 2026-07-28) |
