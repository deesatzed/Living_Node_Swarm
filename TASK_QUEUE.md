...
| Q4-05 | Build relationship inspector | Q3-05, Q4-01 | type, units, lag, evidence, state editable/reviewable | In progress — draft-only reusable inspector exposes complete relationship semantics and warnings. Canonical Edit now distinguishes existing dependency-state controls from proposed relationship additions: an operator selects parent/child factors, supplies typed finite coefficient data, validates it, receives a server-issued structural proposal hash, and can approve only that reviewed binding. Validation receipts explicitly distinguish a clean warning set and are invalidated when the staged contract changes. The current server/UI path is limited to zero-lag additions between existing nodes; structural simulation, project lifecycle sync, node/removal/state-delta approval, full reusable-inspector binding, and E2E proof remain (72 server tests; 96 shared tests; 6 fixture E2E tests; 78 kernel tests, 2026-07-28). |
...
| Q4-07 | Build active-vs-candidate and model/ensemble comparison | Q3-06, Q4-01 | paths and distribution changes visible; explicit two-model weighted ensemble is backend-owned and no false improvement label appears | In progress — Edit stages/removes a graph-derived multi-parameter local candidate set, runs it in memory with paired summaries/affected path/no-improvement limitation, and can persist exact numeric overrides or explicit active/excluded node/dependency-state deltas as non-active version-bound revisions. Run can execute a saved, version-bound numeric scenario and compare/save/reload two operator-specified graph versions as a backend-sampled weighted distribution mixture without a false improvement/recommendation label. A named exact-binding ensemble approval now persists and is read-only reviewable after refresh, without activating an ensemble or mutating a graph. Active graphs now persist relationship metadata, and a server-issued structural relationship-addition proposal validates a zero-lag trial graph without mutation; structural deltas are still not simulated or approvable, and new structural elements and restore/undo remain (2026-07-28) |
