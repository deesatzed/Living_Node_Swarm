...
| Q4-03 | Build target-centered hop graph | Q3-03, Q4-01 | 30-node deterministic layout has no default overlap | In progress — canonical 30-factor deterministic fixture and selected approved Edit graph support no-default-overlap hop layers, pan/zoom/fit, filtering, keyboard selection, three-hop tracing, and textual alternative. The fixture map now also supports remove/extend/request/replay controls with a complete candidate delta and explicit active-graph immutability receipt; that revision remains browser-session fixture state, not persisted/approved structure. Approved-map edge details now use persisted active relationship contracts for type, transform, sign, units, coefficient parameters, lag, evidence claims, and state; missing legacy values remain explicit. Browser proof asserts 30 unique rendered coordinates at both `1440x900` and `1280x800`; graph-node evidence linking remains (2026-07-28) |
...
| Q4-09 | Add component/accessibility tests | Q4-02–08 | keyboard, labels, statuses, reduced motion pass | In progress — graph controls are label/status-covered and the visual graph is now keyboard-focusable with an explicit arrow-key instruction; canonical styles provide visible focus and reduced-motion overrides. Both canonical Build viewports cover cancellation of an unsaved Vet action and an explicit permission-denied routing state before consent. The canonical Project Home fixture additionally runs `@axe-core/playwright` and reports no serious/critical violations. Broader screen-by-screen and assistive-technology coverage remains (2026-07-28). |
