...
| Q4-05 | Build relationship inspector | Q3-05, Q4-01 | type, units, lag, evidence, state editable/reviewable | In progress — Canonical Edit now binds the selected persisted active dependency to the reusable read-only inspector, mapping the stored type, transform, coefficient parameters/units, source/target units, sign, lag, evidence-claim IDs, and state. Missing legacy fields are rendered as `Not recorded`, never substituted with plausible values. Edit also supports typed zero-lag relationship addition, exact persisted relationship removal, and metadata-complete non-target node retirement: each receives a server-issued structural proposal hash, may run only as an in-memory structural comparison, and requires exact reviewed approval. Physical deletion/restoration and generic state-delta approval remain (78 server tests; 122 shared tests; 8 fixture E2E tests; 2026-07-28). |
