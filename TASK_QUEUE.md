...
| Q6-02 | Replace duplicate gas UI with shared workspace preset | Q4-10, Q6-01 | gas app uses shared components | In progress — the Gas preset mounts the shared `WorkspaceShell` and now renders its dynamic dependency map through shared `HopGraph`, including shared zoom/pan/filter/keyboard/textual-dependency behavior and non-color node/edge statuses. Its Gas-specific bootstrap, parameter/simulation detail, and Kalshi-bound controls remain preset-specific; full shared distribution/relationship inspector migration remains. (gas build and safety verifier, 2026-07-28) |
