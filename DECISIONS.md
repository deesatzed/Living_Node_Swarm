...
| D-059 | 2026-07-28 | GUI completion evidence is fail-closed: a repository verifier requires the four canonical screenshots, fixture-labeled E2E/accessibility receipts, and a P01–P25 proof map, while the report records partial and deferred proof honestly. | Accepted | A collection of green commands is not a durable completion surface. The verifier validates evidence presence and declared scope without treating saved fixture artifacts as fresh live-provider or full-accessibility proof. |
| D-060 | 2026-07-28 | The canonical Build browser fixture uses the same 30-factor deterministic candidate shape as the shared graph fixture and asserts unique rendered coordinates at both required desktop viewports. | Accepted | The goal requires browser-level 30-node layout evidence, not only a layout-unit assertion or a smaller screenshot fixture. This remains fixture-only and does not activate proposed factors. |
| D-061 | 2026-07-28 | A Normal/LogNormal median–P90 elicitation result is persisted as a version-bound candidate distribution specification alongside matching numeric overrides; it cannot use the generic numeric approval workflow. | Accepted | The returned specification carries method, evidence links, as-of timestamp, confidence rationale, support, and canonical parameters. A numeric-only approval would lose that governed provenance and falsely imply it had been reviewed for activation. |
| D-062 | 2026-07-28 | All eight canonical families receive server-owned intuitive-input derivation mappings; Normal/LogNormal retain median/P90 receipts, while other families use explicitly named human inputs and return validated canonical parameters plus derived statistics. | Accepted | Browser formulas would duplicate scientific semantics and hide validation. Each response remains an initial assumption with limitations, not a fit, evidence validation, or activation path. |
| D-063 | 2026-07-28 | A provenance-bearing candidate distribution specification requires its own immutable, version-bound approval contract and active-graph provenance persistence; it must not be routed through numeric-only candidate approval. | Accepted | Existing numeric approval binds only override maps and applies only node parameters. Reusing it would discard the reviewed method, evidence links, as-of time, confidence rationale, support, and canonical specification while falsely presenting activation as governed. |
| D-064 | 2026-07-28 | Distribution-spec activation extends the immutable candidate-proposal contract with a typed full specification, whose hash binds matching runtime overrides and whose approval atomically persists both on the active node. | Accepted; supersedes D-061's no-activation limitation | A separate browser-created numeric proposal would lose provenance. Extending the already version-bound proposal preserves compatibility while making ID, family, support, method, evidence links, as-of time, rationale, and canonical parameters part of the reviewed binding. Ordinary parameter edits clear stale active provenance. |
| D-065 | 2026-07-28 | Distribution inspector display quantiles use a fixed-seed, fixed-size canonical-registry Monte Carlo sample and are labeled as such. | Accepted | A uniform all-family quantile surface is needed without browser math or a new external numerical dependency. The display interval is reproducible and read-only, but is not an analytic confidence interval, fitted distribution, calibration result, or forecast-quality claim. |
| D-066 | 2026-07-28 | Persisted relationship inspection displays absent legacy contract fields as `Not recorded`, including an unknown relationship type and no inferred lag. | Accepted | A review surface must not manufacture semantic values for pre-contract graph edges. The active-contract adapter remains read-only; proposal staging and approval are separate controls. |
...
**Consequence:** Map may create a non-active binding for a selected direct factor or for a complete selected path whose relationship count matches the factor’s declared hop distance, and show its hash. It refuses incomplete or mismatched paths and cannot support any accuracy claim from the fixture.
