...
| [P06] Branch revision/refinement delta | Fixture-only | At both required viewports, the canonical Build journey removes a fixture candidate factor, extends the Freight-capacity branch, requests and replays the resulting fixture revision, and renders the complete removed/added-factor and added-dependency delta with `Active graph unchanged: yes`. It then materializes the unedited deterministic fixture, creates an exact server-issued structural binding for a selected direct factor, and requires a named/acknowledged project approval before rendering its graph-v2 receipt. | The replay remains browser-session fixture state. The approval covers one direct fixture scenario-assumption factor only; it is not live research, complete multi-hop selection, or accuracy evidence. |
...
| [P09] Model decision and ensemble | Partial | Build browser proof now covers one direct fixture factor through exact structural binding and named project approval, while saved ensemble configurations require a nonblank operator rationale, preserve it through restart, bind it into the server-issued exact configuration hash, and show it during named review/approval. The canonical Run fixture then loads that exact binding, requests a backend weighted-mixture receipt, and proves the member graphs unchanged. | Build remains fixture-only and direct-factor-only; there is no live evaluation or complete multi-hop decision journey. |
