...
| [P06] Branch revision/refinement delta | Fixture-only | At both required viewports, the canonical Build journey removes a fixture candidate factor, extends the Freight-capacity branch, requests and replays the resulting fixture revision, and renders the complete removed/added-factor and added-dependency delta with `Active graph unchanged: yes`. Map can also materialize the unedited deterministic fixture as a persisted graph with an active target and 15 proposed factors, explicitly for separate review. | The replay remains browser-session fixture state; materialization does not create active candidate factors, structural dependencies, a version-bound approval proposal, or an approved model revision. |
...
| [P09] Model decision and ensemble | Partial | Saved ensemble configurations require a nonblank operator rationale, preserve it through restart, bind it into the server-issued exact configuration hash, and show it during named review/approval. The canonical Run fixture then loads that exact binding, requests a backend weighted-mixture receipt, and proves the member graphs unchanged. | Not a complete new-model decision journey or a live evaluation of either member model. |
