...
| [P06] Branch revision/refinement delta | Partial | Durable revision comparison plus structural Edit component/browser tests; both the selected persisted active dependency and the pre-draft approved graph edge list are read-only inspectable with stored type, units, lag, coefficient parameters, evidence IDs, state, and explicit `Not recorded` legacy values. | Branch extension/replay is not a complete end-to-end new-model journey. |
...
| [P25] Regression surface | Locally verified | Kernel 86, server 89, shared UI 123, canonical Playwright 8, UI/gas builds, and `git diff --check` from this session. | Fresh command output is local, not CI. |
