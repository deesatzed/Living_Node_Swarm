...
| [P05] New Neodymium target and 15 factors | Fixture-only | Canonical fixture Build tests and screenshots at both viewports show every fixture node’s role, hop/state/evidence, Normal family, labeled fixture p05–p95 interval, warning, and monitorability on its card and in the textual alternative. | These are deterministic fixture scenario details, not live Neodymium research or fitted distributions. |
| [P06] Branch revision/refinement delta | Fixture-only | At both required viewports, the canonical Build journey removes a fixture candidate factor, excludes then restores Freight capacity as a proposed candidate without activation, extends the Freight-capacity branch, requests and replays the resulting fixture revision, and renders the complete removed/added-factor and added-dependency delta with `Active graph unchanged: yes`. It then materializes the unedited deterministic fixture, creates an exact server-issued structural binding for the selected three-hop Weather → Freight → Refining → target path, runs its in-memory active-versus-candidate structural comparison with a no-mutation receipt, and only then allows named/acknowledged project approval to render the graph-v2 receipt. | The replay and candidate selection remain browser-session fixture state. The approval covers one fixture scenario-assumption path only; the comparison is structural impact, not live research or accuracy evidence. |
...
| [P16] Non-color status encoding | Fixture-only | Canonical Build asserts node role, state/evidence, distribution/fixture interval, warning, monitorability, and a textual model-dependency list for each rendered fixture edge. The Weather → Freight assertion proves type, transform, sign, lag, source/target units, coefficient units, fixture evidence class, and proposed state. | This proves the candidate fixture graph surface; a full audit of every historical/legacy edge payload remains incomplete. |
